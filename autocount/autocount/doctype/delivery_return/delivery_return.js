...
const addMethod = "autocount.autocount.doctype.delivery_return.delivery_return.add";
const editMethod = "autocount.autocount.doctype.delivery_return.delivery_return.edit";
...
function isItemInChosenTable(frm, selectedData) {
	...
	for (var i = 0; i < arr.length; i++) {
		if (arr[i].delivery_order_no == selectedData.delivery_order_no 
...
function populateItemTable(frm, msg) {
	for (var i = 0; i < msg.length; i++) {
		...
		if (remainingQuantity > 0) {
			...
			newRow.delivery_order_no = data.DeliveryOrderNo;
...
function populateChosenTable(frm, selected) {
	for (var i = 0; i < selected.length; i++) {
		if (!isItemInChosenTable(frm, selected[i])) {
			...
			newRow.delivery_order_no = selected[i].delivery_order_no;
...
frappe.ui.form.on(cur_frm.doctype, {
	...
	debtor_code: function(frm) {
		...
		if (debtor_code) {
			frm.call({
				method: "autocount.autocount.doctype.delivery_return.delivery_return.get_items_by_debtor_autocount",
				args:{
